...
namespace TPV_WINDOWS.Controlador
{
    ...
    public class TPVBase
    {
        ...
        {
            if (ControladorComun.BD!.ContarObjetos<Tarifa>() < 2)
            {
                this._tarifaActual = new Tarifa(0,0,"Tarifa Base PVP", 0, 21, "PVP sin IVA incluido");

                this._tarifaActual.AnadirProducto(_productos[0]!, 600);

                //this._tarifaActual.AnadirProducto(0, 600);
                //ControladorComun.BD!.PersistirObjeto(new Tarifa(0, 0, "Tarifa Base PVP", 0, 21, "PVP sin IVA incluido"));
